...
public class OllamaEmbeddingAdapter implements EmbeddingPort {
  ...
  private float[] parseEmbedding(String body) throws Exception {
    ...
    if (!vectors.isArray() || vectors.size() != 1 || !vectors.get(0).isArray()) {
      throw new EmbeddingException("Ollama response must contain exactly one embedding vector");
    }
    ...
    for (int index = 0; index < dimensions; index++) {
      ...
      if (!value.isNumber()) {
        throw new EmbeddingException("Ollama embedding contains a nonnumeric value");
      }
      float component = value.floatValue();
      if (!Float.isFinite(component)) {
        throw new EmbeddingException("Ollama embedding contains a nonfinite value");
      }
