...
public class WalletOperationService {
  ...
  public <T> T execute(
      UUID user, String action, String key, String normalized, Class<T> type, Supplier<T> posting) {
    requireKey(key);
    for (int attempt = 0; attempt < 2; attempt++) {
      var existing = find(user, action, key);
      if (existing.isPresent()) return replay(existing.get(), normalized, key, type);
      try {
        return posting.get();
        ...
        if (winner.isPresent()) return replay(winner.get(), normalized, key, type);
  ...
  private <T> T replay(WalletOperation operation, String normalized, String key, Class<T> type) {
    if (!operation.getNormalizedRequest().equals(normalized))
