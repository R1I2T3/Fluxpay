...
/**
 * Development-only simulated partner-network transfer rail.
 *
 ...
 * deterministically; payout limits belong to the route configuration, so this rail enforces no
 * amount ceiling.
 */
@Component
...
public class SimulatedPartnerNetworkRail implements TransferRail {

  @Override
  public RailType type() {
    return RailType.PARTNER_NETWORK;
  }
  ...
  public TransferRailResult execute(TransferRailCommand command) {
    ...
    return TransferRailResult.completed("PARTNER-" + command.attemptId(), command.customerFee());
  }
}
