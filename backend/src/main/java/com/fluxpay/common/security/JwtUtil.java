...
public class JwtUtil {
  ...
  public JwtUtil(@Value("${fluxpay.jwt-secret}") String secret) { this.key = Keys.hmacShaKeyFor(secret.getBytes(StandardCharsets.UTF_8)); }
  ...
  public CurrentUser parse(String token) {
    ...
    return new CurrentUser(UUID.fromString(c.getSubject()), c.get("email", String.class), c.get("role", String.class));
  }
}
