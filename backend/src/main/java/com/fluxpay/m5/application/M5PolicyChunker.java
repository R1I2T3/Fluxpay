...
public class M5PolicyChunker {
  ...
  public List<String> chunk(String content) {
    ...
    for (String sentence : content.trim().split("(?<=[.!?])\\s+")) {
      ...
      if (current.length() > 0 && current.length() + normalized.length() + 1 > MAX_CHUNK_CHARACTERS) {
  ...
  private static void addChunk(List<String> chunks, StringBuilder content) {
    ...
    if (chunks.size() == MAX_CHUNKS) {
      throw new IllegalArgumentException("Policy content exceeds the maximum of " + MAX_CHUNKS + " chunks");
    }
