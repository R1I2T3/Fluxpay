...
public class OllamaChatAdapter implements M5ChatPort {
  ...
  public String answer(String question, List<CopilotSource> sources) {
    try {
      ...
      if (response.statusCode() < 200 || response.statusCode() >= 300) throw new M5ChatException("Ollama chat provider returned HTTP " + response.statusCode());
      String content = json.readTree(response.body()).path("message").path("content").asText().trim();
      if (content.isBlank()) throw new M5ChatException("Ollama chat provider returned an empty answer");
      return content;
    } catch (M5ChatException exception) { throw exception;
    } catch (Exception exception) { throw new M5ChatException("Ollama chat provider is unavailable", exception); }
  }

  private static String systemPrompt(List<CopilotSource> sources) {
    String evidence = sources.stream().map(source -> "[" + source.title() + "] " + source.excerpt()).collect(java.util.stream.Collectors.joining("\n"));
