-- M5 cutover after V704. Only the M5 version range/seed wallet roles were changed
-- for the explicitly approved fresh-schema rebuild; do not replay on old history.
...
-- V703's known synthetic prototype cases remain in compliance_cases, inactive.
DECLARE
    unexpected NUMBER;
BEGIN
...
COMMENT ON TABLE compliance_cases IS 'Inactive V701 prototype history; authoritative M5 cases are screening_cases. Do not infer assessment metadata from these rows.';
