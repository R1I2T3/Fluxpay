...
class AuthKycControllerMvcTest {
  ...
  void duplicateCanonicalEmailReturnsConflict() throws Exception {
    when(authService.register(any()))
        .thenThrow(new AuthException(AuthException.EMAIL_EXISTS, "email is already registered"));
  ...
  void staleKycReviewReturnsConflictEnvelope() throws Exception {
    when(kycService.approve(eq(ADMIN_ID), eq(APPLICATION_ID), any()))
        .thenThrow(new KycException(KycException.KYC_CONFLICT, "KYC application has changed"));
