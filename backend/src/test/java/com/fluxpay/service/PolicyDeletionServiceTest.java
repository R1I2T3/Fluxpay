...
class PolicyDeletionServiceTest {
  ...
  void removesVectorStateBeforeRemovingThePolicyDocument() {
    ...
    PolicyIndexStore indexStore = mock(PolicyIndexStore.class);
    PolicyDocument document = mock(PolicyDocument.class);
    when(documents.findById(policyId)).thenReturn(Optional.of(document));

    new PolicyDeletionService(documents, indexStore).delete(policyId);
  ...
  void refusesToDeleteAMissingPolicy() {
    ...
    assertThatThrownBy(
            () ->
                new PolicyDeletionService(documents, mock(PolicyIndexStore.class)).delete(policyId))
