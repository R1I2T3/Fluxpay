...
**Architecture:** The current branch owns payment confirmation, wallets, KYC, routing, authentication, and messaging. M5 is introduced below `com.fluxpay.m5`, with an adapter implementing the existing `ComplianceAssessor` port; it does not import the source branch's competing M3 payment workflow. M5 adds only new migrations and preserves the existing 1536-dimensional policy vector column, using Ollama `qwen3-embedding:4b` at runtime.
...
- Production uses Ollama `qwen3-embedding:4b` at exactly 1536 dimensions. Deterministic embeddings exist only in test configuration.
...
### Task 1: Create the additive M5 vector-generation schema

**Files:**
- Create: `backend/src/main/resources/db/migration/V604__m5_ollama_vector_space.sql`
- Test: `backend/src/test/java/com/fluxpay/m5/infrastructure/persistence/M5VectorMigrationContractTest.java`

**Schema:** Policy chunks are bound to a generation and retain their existing `VECTOR(1536, FLOAT32)` column. Compliance cases remain in the current `compliance_cases` table; the migration changes no payment state.
...
@Test void m5MigrationsFollowV603() {
  assertThat(migrationNames()).containsSubsequence(
      ...
      "V603__m5_seed_data.sql", "V604__m5_ollama_vector_space.sql");
}
@Test void vectorsUseOllamaDimensions() {
  assertThat(sql("V604__m5_ollama_vector_space.sql"))
      .contains("generation_id RAW(16)");
}
```

- [x] **Step 2: Run** the focused contract test and verify failure.
- [x] **Step 3: Add `policy_generations`, generation-bound policy chunks, and active-generation foreign keys without changing the existing vector dimension.**
- [x] **Step 4: Run** `./mvnw -f backend/pom.xml -Dtest=M5VectorMigrationContractTest,MigrationContractTest test`.
- [ ] **Step 5: Commit** the migration and its focused test with message `feat(m5): add vector generation schema`.
...
**Interfaces:** `EmbeddingPort.embedDocument(String)` and `embedQuery(String)` return validated `float[1536]`. `PolicySearchPort.search(float[], String, int, double)` returns only cited chunks in the active named vector space.

- [ ] **Step 1: Write failing adapter tests.**

```java
@Test void queryUsesOllamaAndPrefix() {
  assertThat(adapter.embedQuery("what is KYC")).hasSize(1536);
...
- [ ] **Step 3: Implement `POST /api/embed`, pass `dimensions: 1536`, apply finite HTTP timeouts and response-size limits, validate finite/nonzero vectors, and use Qwen retrieval instructions for documents and queries.**
...
- [ ] **Step 3: Set defaults for `fluxpay.m5.enabled=true`, `fluxpay.m5.ollama.url=http://localhost:11434/api/embed`, `fluxpay.m5.ollama.model=qwen3-embedding:4b`, and `fluxpay.m5.ollama.dimensions=1536`.**
...
- [ ] **Step 1: Document exactly `ollama pull qwen3-embedding:4b`, `ollama serve`, M5 properties, policy indexing, and the current extractive-Copilot limitation.**
