...
USERS=[("alice@demo.io","Pass123!","ADMIN"),("bob@demo.io","Pass123!","USER"),("carol@demo.io","Pass123!","USER")]
...
def main():
    ap=argparse.ArgumentParser(); ap.add_argument("--reset",action="store_true"); ap.add_argument("--env-file",default=".env"); ap.add_argument("--verbose",action="store_true"); a=ap.parse_args()
    ...
    for email,pw,role in USERS:
        ...
        if a.verbose: print("register",email,s,b[:120])
        s2,b2=post("/api/auth/login",{"email":email,"password":pw})
        if s2==200: tokens.append(b2)
