...
(()=>{
 ...
 const quotes=()=>({expiresAt:new Date(Date.now()+300000).toISOString(),quotes:[{id:'q-1',route:'BANK_TRANSFER',recommended:true,feeAmount:'1.50',offeredRate:'83.8',recipientAmount:'10349.30',estimatedMinutes:30},{id:'q-2',route:'EXPRESS_TRANSFER',recommended:false,feeAmount:'3.00',offeredRate:'83.6',recipientAmount:'10199.20',estimatedMinutes:10}]});
 ...
 window.fetch=async(url,options={})=>{
   ...
   if(p==='/api/users/me')data=user;
   ...
   else if(p==='/api/wallets/receive-demo'){const wallet=wallets.find(w=>w.currency===body.currency);wallet.availableBalance=String(Number(wallet.availableBalance)+Number(body.amount));entries.unshift({entryId:crypto.randomUUID(),entryType:'CREDIT',currency:body.currency,amount:body.amount,journalReference:'wallet:demo:new',narration:'QA top-up',createdAt:new Date().toISOString()});data=wallet;}
   else if(p==='/api/wallets/convert')data={creditedAmount:'83.50',to:body.to,from:body.from,fee:'0.50'};
     ...
     else if(parts[4]==='submit-payout'){payment.status='COMPLETED';data={status:'COMPLETED',routeCode:body.routeCode,providerRef:'SYNTHETIC-QA-ONLY',allowed:[]};}
