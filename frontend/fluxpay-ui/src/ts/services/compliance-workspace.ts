...
const importError = (message: string): never => {
...
export function parsePolicyImport(raw: unknown): PolicyDraft[] {
  ...
  if (typeof root === 'string') {
    try {
      ...
      return importError('Policy import must contain valid JSON.');
    }
  }
  if (!root || typeof root !== 'object')
    return importError('Policy import must be a policy object or array.');
  const entries = Array.isArray(root) ? root : [root];
  const duplicates = new Set<string>();
  return entries.map((entry, index) => {
    if (!entry || typeof entry !== 'object' || Array.isArray(entry))
      return importError(`Policy ${index + 1} must be an object.`);
    const { title, category, content } = entry as {
      title?: unknown;
      category?: unknown;
      content?: unknown;
    };
    if (typeof title !== 'string' || typeof category !== 'string' || typeof content !== 'string')
      return importError(`Policy ${index + 1} requires a title, category, and content.`);
    const trimmedTitle = title.trim(),
      trimmedCategory = category.trim(),
      trimmedContent = content.trim();
    if (!trimmedTitle || !trimmedContent)
      return importError(`Policy ${index + 1} requires a non-blank title and content.`);
    if (trimmedTitle.length > 200)
      return importError(`Policy ${index + 1} title must be 200 characters or fewer.`);
    if (!policyCategories.includes(trimmedCategory))
      return importError(`Policy ${index + 1} has an invalid category.`);
    const key = normalizePolicyText(trimmedTitle) + '\u0000' + normalizePolicyText(trimmedContent);
    if (duplicates.has(key))
      return importError(`Policy ${index + 1} duplicates another policy title and content.`);
    duplicates.add(key);
...
export class ComplianceWorkspace {
  ...
  policyEditClearChunks = ko.pureComputed<boolean>({
    ...
    write: (value: boolean) => {
      ...
      if (draft) draft.clearExistingChunks(value);
    },
  });
  ...
  caseSort = ko.observable<'created-desc' | 'created-asc' | 'risk-desc' | 'risk-asc'>(
    'created-desc',
  );
  ...
  confirmation = ko.observable<
    'approve' | 'reject' | 'delete-case' | 'delete-policy' | 'delete-chunk' | 'index' | ''
  >('');
  question = ko.observable('');
  copilotPaymentId = ko.observable('');
  answer = ko.observable<CopilotAnswer>();
  answeredQuestion = ko.observable('');
  filteredPolicies = ko.pureComputed(() =>
    this.policies().filter(
      (p) =>
        (this.categoryFilter() === 'ALL' || p.category === this.categoryFilter()) &&
        (p.title + ' ' + p.content).toLowerCase().includes(this.search().toLowerCase()),
    ),
  );
  pagedPolicies = ko.pureComputed(() =>
    this.filteredPolicies().slice(
      this.policyPage() * this.policyPageSize,
      (this.policyPage() + 1) * this.policyPageSize,
    ),
  );
  policyPageCount = ko.pureComputed(() =>
    Math.max(1, Math.ceil(this.filteredPolicies().length / this.policyPageSize)),
  );
  filteredCases = ko.pureComputed(() => {
    const rank: { [key: string]: number } = { LOW: 1, MEDIUM: 2, HIGH: 3 };
    const result = this.cases().filter(
      (c) =>
        (this.status() === 'ALL' || c.status === this.status()) &&
        (this.riskFilter() === 'ALL' || c.risk === this.riskFilter()) &&
        (this.originFilter() === 'ALL' ||
          (this.originFilter() === 'PAYMENT_REVIEW' ? !!c.reviewReference : !c.reviewReference)) &&
        (c.id + ' ' + c.paymentId + ' ' + c.risk + ' ' + c.riskReasons.join(' '))
          .toLowerCase()
          .includes(this.search().toLowerCase()),
    );
    return result.sort((a, b) =>
      this.caseSort().startsWith('risk')
        ? (rank[a.risk] - rank[b.risk]) * (this.caseSort() === 'risk-desc' ? -1 : 1)
        : (new Date(a.createdAt).getTime() - new Date(b.createdAt).getTime()) *
          (this.caseSort() === 'created-desc' ? -1 : 1),
    );
  });
  pagedCases = ko.pureComputed(() =>
    this.filteredCases().slice(
      this.casePage() * this.casePageSize,
      (this.casePage() + 1) * this.casePageSize,
    ),
  );
  casePageCount = ko.pureComputed(() =>
    Math.max(1, Math.ceil(this.filteredCases().length / this.casePageSize)),
  );
  completedCases = ko.pureComputed(() => this.cases().filter((item) => item.status !== 'OPEN'));
  canDeleteCase = ko.pureComputed(
    () => this.selectedCase()?.status === 'OPEN' && !this.selectedCase()?.reviewReference,
  );
  ...
  private clear() {
    this.policies([]);
    this.policyDrafts([]);
    this.draftEditor(undefined);
    this.draftEditorTarget(undefined);
    this.draftViewer(undefined);
    this.policyEdit(undefined);
    this.chunkEdit(undefined);
    this.chunkEditContent('');
    ...
    this.chunks([]);
    this.guidance([]);
    this.guidanceCaseViewer(undefined);
  ...
  nextPolicyPage = () =>
    this.policyPage(Math.min(this.policyPageCount() - 1, this.policyPage() + 1));
  loadCases = () =>
    this.run(async () => {
      this.cases(await api.complianceCases('ALL'));
      this.casePage(0);
    });
  ...
  toggleRiskSort = () => {
    this.caseSort(this.caseSort() === 'risk-desc' ? 'risk-asc' : 'risk-desc');
    this.casePage(0);
  };
  toggleCreatedSort = () => {
    this.caseSort(this.caseSort() === 'created-desc' ? 'created-asc' : 'created-desc');
    this.casePage(0);
  };
  openPolicy = (p: { id: string }) => this.run(() => this.readPolicy(p.id));
  private async readPolicy(id: string) {
    const [p, c, g, allCases] = await Promise.all([
      api.policy(id),
      api.policyChunks(id),
      api.policyGuidance(id),
      api.complianceCases('ALL'),
    ]);
    this.policy(p);
    this.chunks(c);
    this.guidance(g);
  ...
  loadPolicyJson = (contextOrEvent: unknown, event?: Event) => {
    ...
    this.policyImportError('');
    if (!files.length) {
      this.policyImportError('Choose one or more JSON policy files.');
      return;
    }
    input.value = '';
    ...
    if (jsonFiles.length !== files.length)
      this.policyImportError('Only JSON policy files were imported; non-JSON files were skipped.');
    const read = (file: File) =>
      new Promise<PolicyDraft[]>((resolve, reject) => {
        const reader = new FileReader();
        reader.onerror = () => reject(new Error(`Unable to read ${file.name}.`));
        reader.onload = () => {
          try {
            resolve(parsePolicyImport(reader.result));
          } catch (e: any) {
            reject(new Error(`${file.name}: ${e.message || 'Unable to import policy JSON.'}`));
          }
        };
    ...
    void Promise.allSettled(jsonFiles.map(read)).then((results) => {
      ...
      if (errors.length)
        this.policyImportError([this.policyImportError(), ...errors].filter(Boolean).join(' '));
    });
  };
  addManualPolicyDraft = () => {
    if (!this.busy()) {
      this.policyImportError('');
      this.draftEditorTarget(undefined);
      this.draftEditor(new PolicyDraft());
    }
  };
  editPolicyDraft = (draft: PolicyDraft) => {
    if (!this.busy()) {
      this.draftEditorTarget(draft);
      this.draftEditor(
        new PolicyDraft({
          id: draft.id,
          title: draft.title(),
  ...
  viewPolicyDraft = (draft: PolicyDraft) => {
    if (!this.busy()) this.draftViewer(draft);
  };
  savePolicyDraft = () => {
    const editor = this.draftEditor();
    if (!editor) return false;
    try {
      this.policyDraftPayload(editor);
    } catch (e: any) {
      editor.error(e.message);
      return false;
    }
    const target = this.draftEditorTarget();
    if (target) {
      target.title(editor.title());
      target.category(editor.category());
      target.content(editor.content());
    } else this.policyDrafts.push(editor);
    this.draftEditor(undefined);
    this.draftEditorTarget(undefined);
    return false;
  };
  closePolicyDraftEditor = () => {
    if (!this.busy()) {
      this.draftEditor(undefined);
      this.draftEditorTarget(undefined);
    }
  ...
  private policyDraftPayload(draft: PolicyDraft) {
    ...
    if (!title || title.length > 200 || !content || !this.categories.includes(category))
      throw new Error('Enter a title (up to 200 characters), category and policy content.');
  ...
  createPolicyDrafts = () =>
    this.run(async () => {
      ...
      for (const draft of this.policyDrafts()) {
        try {
          draft.error('');
          await api.createPolicy(this.policyDraftPayload(draft));
          completed.add(draft.id);
        } catch (e: any) {
          draft.error(e.message || 'The policy could not be created.');
        }
      }
      if (completed.size) {
        this.policyDrafts(this.policyDrafts().filter((draft) => !completed.has(draft.id)));
        this.policies(await api.policies());
        if (this.policyDrafts().length === 0) {
          this.policyImportError('');
          this.policy(undefined);
          this.policyView('policy');
          this.notice('Policies have been added to the policy library.');
        }
      }
    });
  openPolicyEdit = (policy: Pick<PolicyDocument, 'id' | 'title' | 'category' | 'content'>) => {
    if (!this.busy()) {
      this.error('');
      const draft = new PolicyDraft(policy);
      this.policyEdit(draft);
      this.policyEditSnapshot(
        JSON.stringify({
          title: draft.title(),
          category: draft.category(),
          content: draft.content(),
          clearExistingChunks: draft.clearExistingChunks(),
        }),
      );
    }
  };
  savePolicyEdit = () =>
    this.run(async () => {
      ...
      try {
        draft.error('');
        body = {
          ...this.policyDraftPayload(draft),
          clearExistingChunks: draft.clearExistingChunks(),
        };
      } catch (e: any) {
        draft.error(e.message || 'Enter valid policy details.');
        return;
      }
      try {
        const updated = await api.updatePolicy(draft.id, body);
        this.policy(updated);
        if (body.clearExistingChunks) this.chunks([]);
        this.policyEdit(undefined);
        this.policies(await api.policies());
        this.notice(
          body.clearExistingChunks
            ? 'Policy updated. Rebuild its index before asking Copilot.'
            : 'Policy updated. Existing chunks and index were retained.',
        );
      } catch (e: any) {
        draft.error(e.message || 'The policy could not be updated.');
      }
    });
  closePolicyEdit = () => {
    const draft = this.policyEdit();
    ...
    if (current !== this.policyEditSnapshot() && !window.confirm('Discard unsaved policy changes?'))
      return;
    this.policyEdit(undefined);
    this.policyEditSnapshot('');
  };
  openAdvancedFromEdit = () => {
    ...
    if (
      current !== this.policyEditSnapshot() &&
      !window.confirm('Discard unsaved policy changes and open advanced settings?')
    )
      return;
    this.policyEdit(undefined);
    this.policyEditSnapshot('');
    this.policyView('advanced');
  };
  newPolicy = () => {
    this.policy(undefined);
    this.title('');
    this.content('');
    this.category('PAYMENT_REVIEW');
  ...
  savePolicy = () =>
    this.run(async () => {
      ...
      if (!title || title.length > 200 || !content || !this.categories.includes(this.category()))
        throw new Error('Enter a title (up to 200 characters), category and policy content.');
  ...
  addChunk = () =>
    this.run(async () => {
      const content = this.chunkContent().trim(),
        p = this.policy();
      if (!p || !content) throw new Error('Enter text for the new chunk.');
      await api.addPolicyChunk(p.id, content);
      this.chunkContent('');
      this.notice(
        'Chunk added. Indexing rebuilds chunks from the original document, not these manual additions.',
      );
      await this.readPolicy(p.id);
    });
  openChunkEdit = (chunk: PolicyChunk) => {
    if (!this.busy() && chunk.manual) {
      this.error('');
      this.chunkEdit(chunk);
      this.chunkEditContent(chunk.content);
    }
  };
  closeChunkEdit = () => {
    if (!this.busy()) {
  ...
  saveChunkEdit = () =>
    this.run(async () => {
      const policy = this.policy(),
        chunk = this.chunkEdit(),
        content = this.chunkEditContent().trim();
      if (!policy || !chunk || !chunk.manual)
        throw new Error('Only manually added chunks can be edited.');
      if (!content) throw new Error('Chunk text cannot be blank.');
      await api.updatePolicyChunk(policy.id, chunk.id, content);
      this.closeChunkEdit();
      await this.readPolicy(policy.id);
      this.notice('Manual chunk updated.');
    });
  requestDeleteChunk = (chunk: PolicyChunk) => {
    if (!this.busy()) {
      this.error('');
      this.pendingChunk(chunk);
      this.confirmation('delete-chunk');
    }
  };
  addGuidance = () =>
    this.run(async () => {
      const policy = this.policy(),
        caseId = this.guidanceCaseId().trim(),
        content = this.guidanceContent().trim();
      if (!policy || !this.uuid(caseId) || !content)
        throw new Error('Choose a completed case ID and enter guidance text.');
      await api.addPolicyGuidance(policy.id, { complianceCaseId: caseId, content });
      this.guidanceCaseId('');
      this.guidanceContent('');
      await this.readPolicy(policy.id);
      this.notice('Guidance added. Rebuild the search index to make it available to Copilot.');
    });
  openGuidanceEdit = (item: PolicyGuidance) => {
    if (!this.busy()) {
      this.guidanceEdit(item);
      this.guidanceEditContent(item.content);
    }
  };
  closeGuidanceEdit = () => {
    if (!this.busy()) {
      this.guidanceEdit(undefined);
      this.guidanceEditContent('');
    }
  };
  saveGuidanceEdit = () =>
    this.run(async () => {
      const policy = this.policy(),
        item = this.guidanceEdit(),
        content = this.guidanceEditContent().trim();
      if (!policy || !item || !content) throw new Error('Guidance text cannot be blank.');
      await api.updatePolicyGuidance(policy.id, item.id, content);
      this.closeGuidanceEdit();
      await this.readPolicy(policy.id);
      this.notice('Policy guidance updated. Rebuild the index to update Copilot.');
    });
  deleteGuidance = (item: PolicyGuidance) =>
    this.run(async () => {
      const policy = this.policy();
      if (!policy || !window.confirm('Delete this policy guidance?')) return;
      await api.deletePolicyGuidance(policy.id, item.id);
      await this.readPolicy(policy.id);
      this.notice('Policy guidance deleted.');
    });
  ...
  private openGuidanceCaseById(caseId: string) {
    ...
    if (loaded) {
      this.guidanceCaseViewer(loaded);
      return;
    }
    this.run(async () => {
      const item = await api.complianceCase(caseId);
      if (!item?.id) throw new Error('The completed compliance case could not be loaded.');
      this.guidanceCaseViewer(item);
    });
  }
  closeGuidanceCase = () => {
    if (!this.busy()) this.guidanceCaseViewer(undefined);
  };
  openCase = (c: { id: string }) =>
    this.run(async () => {
      this.selectedCase(await api.complianceCase(c.id));
      this.decisionReason('');
      this.confirmation('');
    });
  closeCase = () => {
  ...
  saveCase = () =>
    this.run(async () => {
      ...
      if (
        ...
        !riskReasons.length ||
        !suggestedAction ||
        suggestedAction.length > 400 ||
        !['LOW', 'MEDIUM', 'HIGH'].includes(this.risk())
      )
        throw new Error(
          'Enter a valid payment UUID, at least one reason, and a suggested action up to 400 characters.',
        );
      this.selectedCase(
        await api.createComplianceCase({
          paymentId,
          risk: this.risk(),
          riskReasons,
          suggestedAction,
      ...
      this.status('OPEN');
      this.notice(
        'Manual case created. Decisions on manual cases do not change the payment state.',
      );
      this.cases(await api.complianceCases(this.status()));
    });
  askConfirmation = (action: 'approve' | 'reject' | 'delete-case' | 'delete-policy' | 'index') => {
    if (!this.busy()) {
      this.error('');
  ...
  confirm = () =>
    this.run(async () => {
      ...
      if (action === 'approve' || action === 'reject') {
        ...
        if (this.decisionReason().trim().length > 500)
          throw new Error('Decision notes must be 500 characters or fewer.');
        this.selectedCase(
          await api.decideComplianceCase(c.id, action, this.decisionReason().trim()),
        );
        this.confirmation('');
        this.notice(
          c.reviewReference
            ? 'Decision saved. The linked payment has been updated.'
            : 'Decision saved. This manual case does not change payment state.',
        );
        this.cases(await api.complianceCases(this.status()));
      } else if (action === 'delete-case') {
        if (!c || !this.canDeleteCase()) throw new Error('Only open manual cases can be deleted.');
        await api.deleteComplianceCase(c.id);
        this.selectedCase(undefined);
        this.confirmation('');
        this.notice('Manual case deleted.');
        this.cases(await api.complianceCases(this.status()));
        ...
        this.confirmation('');
        this.answer(undefined);
        this.notice('Policy and its indexed content deleted.');
        this.policies(await api.policies());
      } else if (action === 'delete-chunk') {
        const chunk = this.pendingChunk();
        if (!p || !chunk) throw new Error('Select a policy chunk to delete.');
        await api.deletePolicyChunk(p.id, chunk.id);
        this.pendingChunk(undefined);
        this.confirmation('');
        await this.readPolicy(p.id);
        this.notice(
          chunk.manual
            ? 'Manual chunk deleted.'
            : 'Indexed chunk deleted. Rebuilding the index will restore indexed chunks from the policy document.',
        );
      } else if (action === 'index') {
        if (!p) throw new Error('Select a policy first.');
        const result = await api.indexPolicy(p.id);
        this.confirmation('');
        this.notice('Index published: ' + result.chunkCount + ' chunks.');
        await this.readPolicy(p.id);
      }
    });
  ask = () =>
