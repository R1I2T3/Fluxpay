...
import { feedbackObservable, ToastKind, notify } from './notifications';

export class Page {
  session = session;
  busy = ko.observable(false);
  protected feedback(kind: ToastKind) {
    return feedbackObservable(kind, () => !this.disposed);
  }
  ...
  async run(action: () => Promise<void>, success = '', successToast = true) {
    ...
    try {
      await action();
      if (success && !this.disposed) this.notice(success);
    } catch (e: any) {
      if (!this.disposed) this.error(e.message || 'Something went wrong. Please try again.');
    } finally {
      this.suppressSuccessToast = false;
      this.busy(false);
    }
  }
  async load() {
  ...
  protected async acceptQuote() {
    ...
    if (this.payment()?.status === 'UNDER_REVIEW' && !this.disposed)
      notify(
        'warning',
  ...
  executeAction = () =>
    this.run(
      async () => {
        ...
        if (action === 'confirm') {
          if (!this.selectedQuote() || !this.quoteValid())
            throw new Error('Choose a current quote.');
          await this.acceptQuote();
          ...
          if (action === 'switch')
            this.outcome(await api.switchRoute(id, requireQuoteRoute(q), q.id));
        }
        this.confirmAction('');
        if (this.screen === 'tracking') await this.inspectData();
      },
