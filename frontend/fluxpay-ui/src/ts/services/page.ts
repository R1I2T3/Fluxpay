...
export class Page {
    ...
    await this.run(async()=>{
      switch(this.screen){
        case 'dashboard': {
          ...
          this.wallets(w);this.payments(p.items);this.total(p.total);this.recipients(r);break;
        }
        case 'account': {const user=await api.me();session.user(user);this.email(user.email);this.fullName(user.fullName);break;}
        case 'kyc':this.kyc(await api.kyc());break;
        case 'wallets':this.wallets(await api.wallets());break;
        case 'recipients':this.recipients(await api.recipients());break;
        case 'payments-new': {
          const[w,r]=await Promise.all([api.wallets(),api.recipients()]);
          this.wallets(w);this.recipients(r);this.walletId(w[0]?.walletId||'');this.recipientId(r.find(x=>x.status==='ACTIVE')?.id||'');break;
        }
        case 'payments-list': {
          const[p,r]=await Promise.all([api.payments(this.page()),api.recipients()]);this.payments(p.items);this.total(p.total);this.recipients(r);break;
        }
        case 'tracking': {
          const[p,r,routes]=await Promise.all([api.payments(),api.recipients(),api.routes()]);
          this.payments(p.items);this.recipients(r);this.routes(routes.routes);
          if(this.paymentId())await this.inspectData();break;
        }
        case 'admin':
  ...
  signIn=(form?:HTMLFormElement)=>this.run(async()=>{
    // Read native form values as well: password managers may not emit input/change.
    if(form instanceof HTMLFormElement){const fields=new FormData(form);this.email(String(fields.get('email')||this.email()));this.password(String(fields.get('password')||this.password()));}
    if(!this.email().trim()||!this.password())throw new Error('Enter your email and password.');
    session.set(await api.login({email:this.email().trim(),password:this.password()}));navigate('dashboard');
  });
  signUp=()=>this.run(async()=>{
    if(!this.fullName().trim()||!this.email().trim()||!this.password())throw new Error('Complete all fields.');
    session.set(await api.register({fullName:this.fullName().trim(),email:this.email().trim(),password:this.password()}));navigate('kyc');
  });
  ...
  submitKyc=()=>this.run(async()=>{
    ...
    this.kyc(await api.submitKyc({docType:this.docType(),docNumber:this.docNumber().trim(),documents:this.documents()}));
    await session.restore();
  ...
  ledgerNext=()=>this.run(async()=>{this.ledgerPage(this.ledgerPage()+1);await this.loadLedger();});
  addRecipient=()=>{this.recipientEditing(undefined);this.recipientName('');this.account('');this.bankName('');this.country('IN');this.recipientCurrency('INR');this.recipientStatus('ACTIVE');this.recipientFormOpen(true);};
  editRecipient=(r:any)=>{this.recipientEditing(r);this.recipientName(r.name);this.account(r.account);this.bankName(r.bankName);this.country(r.country);this.recipientCurrency(r.currency);this.recipientStatus(r.status);this.recipientFormOpen(true);};
  closeRecipient=()=>this.recipientFormOpen(false);
  saveRecipient=()=>this.run(async()=>{
    const b={name:this.recipientName().trim(),account:this.account().trim(),bankName:this.bankName().trim(),country:this.country().toUpperCase(),currency:this.recipientCurrency(),status:this.recipientStatus(),expectedVersion:this.recipientEditing()?.version};
    if(!b.name||!b.account||!b.bankName||!/^[A-Z]{2}$/.test(b.country))throw new Error('Complete the recipient details and use a two-letter country code.');
    if(this.recipientEditing())await api.updateRecipient(this.recipientEditing().id,b);else await api.recipient(b);
    this.recipients(await api.recipients());this.recipientFormOpen(false);
  },'Recipient saved.');
  async loadQuotes(generate=false){
    const data=generate?await api.quotes(this.paymentId()):await api.getQuotes(this.paymentId());
    this.quotes(data.quotes||[]);this.quoteExpires(data.expiresAt);this.selectedQuote(undefined);
  }
  createDraft=()=>this.run(async()=>{
    ...
    const p=await api.draft({sourceWalletId:wallet.walletId,recipientId:recipient.id,sourceAmount:this.validAmount(this.amount()),sourceCurrency:wallet.currency,payoutCurrency:recipient.currency,purpose:this.purpose(),preference:this.preference()});
    this.payment(p);this.paymentId(p.id);this.step(2);await this.loadQuotes(true);
  });
  refreshQuotes=()=>this.run(async()=>{await this.loadQuotes(true);});
  chooseQuote=(quote:any)=>{this.selectedQuote(quote);this.confirmAction('confirm');};
  ...
  nextPage=()=>{this.page(this.page()+1);void this.load();};
  previousPage=()=>{this.page(Math.max(0,this.page()-1));void this.load();};
  inspect=()=>this.run(()=>this.inspectData());
  async inspectData(){
    if(!this.paymentId())throw new Error('Select or enter a payment ID.');
    const p=await api.payment(this.paymentId());this.payment(p);
    this.timeline(await api.timeline(this.paymentId()));
    this.quotes([]);this.selectedQuote(undefined);
    if(['QUOTED','PROCESSING','FAILED','UNDER_REVIEW','COMPLETED'].includes(p.status))await this.loadQuotes();
    clearInterval(this.polling);
    if(['PROCESSING','UNDER_REVIEW'].includes(p.status))this.polling=window.setInterval(()=>{
      if(!this.disposed&&!this.busy()&&document.visibilityState==='visible'){
        Promise.all([api.payment(this.paymentId()),api.timeline(this.paymentId())]).then(([p,t])=>{this.payment(p);this.timeline(t);if(!['PROCESSING','UNDER_REVIEW'].includes(p.status))clearInterval(this.polling);}).catch(()=>clearInterval(this.polling));
      }
    },10000);
  }
  recommend=()=>this.run(async()=>{this.recommendation(await api.recommend(this.paymentId(),this.preference()));});
  askAction=(action:string)=>{this.confirmAction(action);};
  executeAction=()=>this.run(async()=>{
